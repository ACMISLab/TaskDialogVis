...
from api.openai_4o_mini_api import ChatGPT4oMiniGenerator
...
textgen = ChatGPT4oMiniGenerator()
...
for i, dialogue in enumerate(tqdm(test_data)):
    ...
    for index, chart in enumerate(tqdm(dialogue["dialogues"])):
        ...
        for n in range(3):
            ...
            print(response)
            chart_dict["charts"].append(response)


        all_of_result[-1]["dialogues"].append(chart_dict)
    with open(r"output/gpt4o_mini_output.json", "w", encoding="utf-8") as file:
